...
on run
	...
	-- 靴選択
	set footwearOptions to {"none", "カスタム", "black pump shoes", "white sneaker shoes", "ankle boot shoes", "flat sandal shoes", "running shoes", "platform shoes", "combat boot shoes", "designer heel shoes", "mary jane shoes", "comfortable nursing shoes", "steel-toe boot shoes", "polished dress shoes", "sculptural shoes", "avant-garde heel shoes", "leather boot shoes", "sandal shoes", "loafer shoes", "espadrille shoes", "waterproof boot shoes", "low-heel pump shoes", "non-slip dress shoes", "comfortable flat shoes", "tactical boot shoes", "duty boot shoes", "soft slipper shoes", "fuzzy sock shoes", "yoga socks", "barefoot"}
	
	-- 特殊スタイルの場合は靴の説明を変更
	set footwearPrompt to "靴を選択してください:"
	if mainCat is "特殊スタイル" then
		set footwearPrompt to "靴を選択してください（特殊スタイルの場合は靴が含まれているのでnoneを選択可能）:"
	end if
	
	set selectedFootwearList to choose from list footwearOptions with prompt footwearPrompt default items {"black pump shoes"}
	...
	-- 靴の処理（noneの場合は追加しない）
	if selectedFootwear is not "none" then
		set finalPrompt to finalPrompt & ", and " & selectedFootwear
	end if
	
	set finalPrompt to finalPrompt & ". She stands straight, studio lighting and gray background."
...
on handleSubcultureCategory()
	...
	if subCat is "カスタム" then
		...
		return {selectedOutfit, "elegant frilly fashion"}
	end if
...
on handleMedicalCategory()
	set options to {"カスタム", "a professional white lab coat over scrubs", "a surgical scrubs in teal with medical accessories", "a nurse uniform in navy with medical badge", "a pharmacy coat over professional attire", "a crisp white medical scrubs with stethoscope"}
