-- Simplified Fashion Prompt Generator for Midjourney
-- AppleScript版（エラーハンドリング簡素化）
...
on run
	...
	if selectedMainCategory is false then
	...
	if mainCat is "カスタム" then
		set customResult to text returned of (display dialog "カスタム衣装を入力してください:" default answer "")
		if customResult is "" then
			display dialog "キャンセルされました" buttons {"OK"} default button "OK"
			return
		end if
	...
	else if mainCat is "特殊シーン" then
		set resultList to my handleSpecialCategory()
		if resultList is false then
			display dialog "キャンセルされました" buttons {"OK"} default button "OK"
			return
		end if
		...
		set customResult to text returned of (display dialog "衣装を入力してください:" default answer "")
		if customResult is "" then
			display dialog "キャンセルされました" buttons {"OK"} default button "OK"
			return
		end if
		set selectedOutfit to customResult
		set categoryDisplayName to "other style"
	end if
	
	-- ストッキング・ソックス選択
	set stockingOptions to {"none", "カスタム", "black stocking", "white stocking", "loose socks", "black thigh-high socks", "knee-high socks", "ankle socks"}
	
	set selectedStockingList to choose from list stockingOptions with prompt "ストッキング・ソックスを選択してください（不要な場合はnone）:" default items {"none"}
	
	if selectedStockingList is false then
		display dialog "キャンセルされました" buttons {"OK"} default button "OK"
		return
	end if
	...
	if selectedStocking is "カスタム" then
		set customStocking to text returned of (display dialog "カスタムストッキング・ソックスを入力してください:" default answer "")
		if customStocking is "" then
			display dialog "キャンセルされました" buttons {"OK"} default button "OK"
			return
		end if
	...
	-- アクセサリー選択
	set accessoryOptions to {"none", "カスタム", "headphones", "hair ribbon", "headband", "hair clips", "beret hat", "school hat", "wristwatch", "glasses"}
	
	set selectedAccessoryList to choose from list accessoryOptions with prompt "アクセサリーを選択してください（不要な場合はnone）:" default items {"none"}
	
	if selectedAccessoryList is false then
		display dialog "キャンセルされました" buttons {"OK"} default button "OK"
		return
	end if
	...
	if selectedAccessory is "カスタム" then
		set customAccessory to text returned of (display dialog "カスタムアクセサリーを入力してください:" default answer "")
		if customAccessory is "" then
			display dialog "キャンセルされました" buttons {"OK"} default button "OK"
			return
		end if
	...
	-- 靴選択
	set footwearOptions to {"カスタム", "black pump shoes", "white sneaker shoes", "ankle boot shoes", "flat sandal shoes", "running shoes", "platform shoes", "combat boot shoes", "designer heel shoes", "mary jane shoes", "comfortable nursing shoes", "steel-toe boot shoes", "polished dress shoes", "sculptural shoes", "avant-garde heel shoes", "leather boot shoes", "sandal shoes", "loafer shoes", "loafers", "espadrille shoes", "waterproof boot shoes", "low-heel pump shoes", "non-slip dress shoes", "comfortable flat shoes", "tactical boot shoes", "duty boot shoes", "soft slipper shoes", "fuzzy sock shoes", "yoga socks", "barefoot"}
	
	set selectedFootwearList to choose from list footwearOptions with prompt "靴を選択してください:" default items {"black pump shoes"}
	
	if selectedFootwearList is false then
		display dialog "キャンセルされました" buttons {"OK"} default button "OK"
		return
	end if
	...
	if selectedFootwear is "カスタム" then
		set customFootwear to text returned of (display dialog "カスタム靴を入力してください:" default answer "")
		if customFootwear is "" then
			display dialog "キャンセルされました" buttons {"OK"} default button "OK"
			return
		end if
	...
	-- プロンプト生成（ストッキング・アクセサリー対応）
	set finalPrompt to "Hyperrealistic fashion portrait photography, Japanese model in a perfect body line and smooth hair. She is wearing " & categoryDisplayName & ", " & selectedOutfit
	
	if selectedStocking is not "none" then
		set finalPrompt to finalPrompt & " with " & selectedStocking
	end if
	
	if selectedAccessory is not "none" then
		set finalPrompt to finalPrompt & " with " & selectedAccessory
	end if
	
	set finalPrompt to finalPrompt & ", and " & selectedFootwear & ". She stands straight, studio lighting and gray background."
...
on handleBusinessCategory()
	...
	if subCat is "カスタム" then
		set customResult to text returned of (display dialog "カスタム衣装を入力してください:" default answer "")
		if customResult is "" then return false
		...
		set selectedOutfit to item 1 of selected
		if selectedOutfit is "カスタム" then
			set customResult to text returned of (display dialog "カスタム衣装を入力してください:" default answer "")
			if customResult is "" then return false
			set selectedOutfit to customResult
		end if
		return {selectedOutfit, "formal business"}
		...
		return {selectedOutfit, "business casual"}
		...
		return {selectedOutfit, "creative professional"}
		...
		return {selectedOutfit, "interview formal"}
	end if
...
on handleCasualCategory()
	...
	if subCat is "カスタム" then
		...
		set options to {"カスタム", "a white cotton t-shirt and blue jeans", "an oversized sweater and leggings", "a casual button-down and khaki pants"}
		...
		return {selectedOutfit, "daily casual"}
		...
		return {selectedOutfit, "smart casual"}
		...
		return {selectedOutfit, "street style"}
...
on handleFitnessCategory()
	...
	if subCat is "カスタム" then
		...
		return {selectedOutfit, "gym workout"}
		...
		return {selectedOutfit, "yoga pilates"}
		...
		return {selectedOutfit, "outdoor running"}
	end if
...
on handleFormalCategory()
	...
	if subCat is "カスタム" then
		...
		return {selectedOutfit, "party cocktail"}
		...
		return {selectedOutfit, "formal event"}
		...
		return {selectedOutfit, "casual party"}
		...
		return {selectedOutfit, "date dinner"}
	end if
...
on handleSubcultureCategory()
	...
	if subCat is "カスタム" then
		...
		return {selectedOutfit, "kawaii harajuku"}
		
	else if subCat is "ゴシック・ダーク系" then
		set options to {"カスタム", "a gothic black dress with lace details and corset", "a dark romantic blouse with leather pants and chains", "a gothic lolita dress with petticoat and dark accessories"}
		...
		return {selectedOutfit, "gothic dark"}
		
	else if subCat is "ロリータ系" then
		set options to {"カスタム", "a sweet lolita dress with bows and frills in pastel colors", "a classic lolita blouse with high-waisted skirt and vest", "a gothic lolita dress in black and white with elegant details"}
		...
		return {selectedOutfit, "lolita fashion"}
	end if
end handleSubcultureCategory

-- 制服・ユニフォーム系処理
on handleUniformCategory()
	set options to {"カスタム", "a crisp white medical scrubs with stethoscope", "a navy blue work uniform with utility belt", "a professional pilot uniform with cap and wings pin", "a clean chef's uniform with apron and hat"}
...
on studentUniform()
	...
	if subCat is "カスタム" then
		set customResult to text returned of (display dialog "カスタム学生服を入力してください:" default answer "")
		if customResult is "" then return false
		...
		set selectedOutfit to item 1 of selected
		if selectedOutfit is "カスタム" then
			set customResult to text returned of (display dialog "カスタムセーラー服を入力してください:" default answer "")
			if customResult is "" then return false
			set selectedOutfit to customResult
		end if
		return {selectedOutfit, "sailor uniform"}
		...
		set selectedOutfit to item 1 of selected
		if selectedOutfit is "カスタム" then
			set customResult to text returned of (display dialog "カスタムブレザー制服を入力してください:" default answer "")
			if customResult is "" then return false
			set selectedOutfit to customResult
		end if
		return {selectedOutfit, "blazer uniform"}
		...
		set selectedOutfit to item 1 of selected
		if selectedOutfit is "カスタム" then
			set customResult to text returned of (display dialog "カスタム制服セットを入力してください:" default answer "")
			if customResult is "" then return false
			set selectedOutfit to customResult
		end if
		return {selectedOutfit, "school uniform set"}
	end if
end studentUniform
...
on handleSeasonalCategory()
	...
	if subCat is "カスタム" then
		...
		return {selectedOutfit, "spring autumn"}
...
on handleStyleCategory()
	...
	if subCat is "カスタム" then
		...
		return {selectedOutfit, "minimalist"}
		...
		return {selectedOutfit, "bohemian"}
		...
		return {selectedOutfit, "edgy mode"}
...
on handleLuxuryCategory()
	...
	if subCat is "カスタム" then
		...
		return {selectedOutfit, "haute couture"}
		...
		return {selectedOutfit, "high fashion"}
		...
		return {selectedOutfit, "avant-garde"}
	end if
end handleLuxuryCategory

-- 医療・ヘルスケア系処理
on handleMedicalCategory()
	set options to {"カスタム", "a professional white lab coat over scrubs", "a surgical scrubs in teal with medical accessories", "a nurse uniform in navy with medical badge", "a pharmacy coat over professional attire"}
...
on handleMilitaryCategory()
	...
	return {selectedOutfit, "military"}
...
on handleBasicCategory()
	...
	if subCat is "カスタム" then
		...
		return {selectedOutfit, "fitness base"}
		...
		return {selectedOutfit, "nightwear"}
	end if
end handleBasicCategory

-- 特殊スタイル処理
on handleSpecialStyleCategory()
	set subCategories to {"カスタム", "レーシング・モータースポーツ", "宇宙・SF", "アニメ・コスプレ", "時代劇・歴史", "職業コスチューム"}
	...
	if subCat is "カスタム" then
		set customResult to text returned of (display dialog "カスタム特殊スタイルを入力してください:" default answer "")
		if customResult is "" then return false
		return {customResult, "special style"}
		
	else if subCat is "レーシング・モータースポーツ" then
		set options to {"カスタム", "a professional racing suit with helmet and gloves", "a Formula 1 racing suit with sponsor logos", "a motorcycle racing suit with protective gear", "a NASCAR racing suit with team colors"}
		...
		return {selectedOutfit, "racing motorsports"}
		
	else if subCat is "宇宙・SF" then
		set options to {"カスタム", "a futuristic space suit with helmet", "a sleek astronaut suit with NASA patches", "a sci-fi jumpsuit with high-tech elements", "a space exploration suit with life support systems"}
		...
		return {selectedOutfit, "space sci-fi"}
		
	else if subCat is "アニメ・コスプレ" then
		set options to {"カスタム", "a magical girl costume with colorful details", "a ninja outfit with traditional elements", "a maid cafe costume with frilly apron", "a school idol costume with stage accessories"}
		...
		return {selectedOutfit, "anime cosplay"}
		
	else if subCat is "時代劇・歴史" then
		set options to {"カスタム", "a medieval knight armor with chainmail", "a Victorian era dress with corset", "a samurai armor with traditional helmet", "a Renaissance noble costume with elaborate decorations"}
		...
		return {selectedOutfit, "historical period"}
		
	else if subCat is "職業コスチューム" then
		set options to {"カスタム", "a professional chef costume with tall hat", "a firefighter uniform with safety gear", "a deep-sea diving suit with helmet", "a hazmat suit with protective equipment"}
		set selected to choose from list options with prompt "職業コスチューム衣装を選択してください:" default items {item 2 of options}
		if selected is false then return false
		set selectedOutfit to item 1 of selected
		if selectedOutfit is "カスタム" then
			set customResult to text returned of (display dialog "カスタム衣装を入力してください:" default answer "")
		...
		return {selectedOutfit, "professional costume"}
	end if
